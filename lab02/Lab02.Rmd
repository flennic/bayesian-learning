...
predict_using_model_params = function(time, mu_0, omega_0, nu_0, sigma_sq_0) {
  ...
  # Now we also need an error
  error = rnorm(n = length(time), mean = 0, sd = sigma_sq)
  
  # Then we fit the model
  temp =
    matrix(c(rep(1, length(time)), time, time^2), ncol = 3) %*%
    matrix(beta) +
    error
...
plot = ggplot(df) +
  ...
  plot = plot + labs(title = "Day of Year VS Draft Number",
