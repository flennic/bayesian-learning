...
sample_model_params = function(X, mu_0, omega_0, nu_0, sigma_sq_0) {
...
predict_using_model_params = function(X, beta) {
...
for (i in 3:(m+2)) {
  model_params = sample_model_params(X, mu_0, omega_0, nu_0, sigma_sq_0)
  priors[,i] = predict_using_model_params(X, model_params$beta)
}
...
2. $\beta|\sigma^2 \sim \mathcal{N}(\mu_0, \sigma^2\Omega^{-1}_0)$
...
mu_n = solve(t(X) %*% X + omega_0[1,1]) %*% (t(X) %*% X %*% beta + omega_0 %*% mu_0)
omega_n = t(X) %*%X + omega_0
nu_n = nu_0 + n
nu_n_and_sigma_n = nu_0 * sigma_sq_0 + (t(Y) %*% Y + t(mu_0) %*% omega_0 %*% mu_0 -
                                          t(mu_n) %*% omega_n %*% mu_n)
